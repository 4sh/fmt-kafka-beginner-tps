    ...
    private static Topology getTopology() {
        ...
        final KStream<String, PositionString> positionsString = positionsFiltered.mapValues(
            value -> {
                ...
                final String positionString = String.format("Latitude: %s, Longitude: %s", latitude, longitude);
                return new PositionString(latitude, longitude, positionString);
            }
        );

        final KStream<String, PositionString> positionPrinted = positionsString.peek((key, value) -> {
            logger.info(
                "Transformed Key:{} Position:{}",
                key,
                value.getPositionString()
            );
        });
